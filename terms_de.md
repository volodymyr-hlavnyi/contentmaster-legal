[Datenschutzerklärung](./index.md)

🌐 Sprachen:
[English](./terms.md) | [Русский](./terms_ru.md) | [Українська](./terms_uk.md) | [Deutsch](./terms_de.md)

# Nutzungsbedingungen — @KontentMasterBot

**Gültig ab:** 26/02/2026

Diese Nutzungsbedingungen („Bedingungen“) regeln die Nutzung des Telegram-Bots @KontentMasterBot („Service“), betrieben
von ContentMaster („wir“, „uns“, „unser“).

Durch die Nutzung des Service stimmen Sie diesen Bedingungen zu.

---

## 1. Beschreibung des Service

ContentMaster bietet KI-gestützte Bildvisualisierungsdienste an.

Nutzer können Bilder hochladen und stilisierte visuelle Ergebnisse erhalten, die mithilfe von Technologien der
künstlichen Intelligenz erzeugt werden.

Der Service kann kostenlose und kostenpflichtige Funktionen enthalten.

Wir behalten uns das Recht vor, den Service jederzeit zu ändern, auszusetzen oder einzustellen.

---

## 2. Teilnahmevoraussetzungen

Sie müssen mindestens 13 Jahre alt sein oder das in Ihrer Rechtsordnung geltende Mindestalter erreicht haben.

Durch die Nutzung des Service bestätigen Sie, dass Sie diese Voraussetzung erfüllen.

---

## 3. Verantwortung des Nutzers

Sie erklären sich damit einverstanden, dass Sie:

- Eigentümer der hochgeladenen Inhalte sind oder über die erforderlichen Nutzungsrechte verfügen
- keine rechtswidrigen, schädlichen oder verletzenden Inhalte hochladen
- den Service nicht missbrauchen oder manipulieren
- den Service nicht für rechtswidrige Zwecke nutzen

Sie tragen die alleinige Verantwortung für die von Ihnen hochgeladenen Inhalte.

---

## 4. Verbotene Inhalte

Der Service darf nicht verwendet werden zur Erstellung oder Verbreitung von:

- rechtswidrigen Inhalten
- urheberrechtsverletzenden Materialien
- verleumderischen Inhalten
- Inhalten, die Persönlichkeitsrechte verletzen
- Hassrede oder missbräuchlichen Inhalten
- betrügerischen oder irreführenden Inhalten

Wir behalten uns das Recht vor, den Zugang bei Verstößen einzuschränken.

---

## 5. Haftungsausschluss für KI-generierte Inhalte

Die Ergebnisse werden automatisch durch KI-Systeme erzeugt.

Wir übernehmen keine Gewähr für:

- Richtigkeit
- faktische Genauigkeit
- kommerzielle Eignung
- rechtliche Zulässigkeit in Ihrer Rechtsordnung

Sie sind verpflichtet, generierte Inhalte vor der Nutzung selbst zu prüfen.

---

## 6. Zahlungen und Credits

Bestimmte Funktionen sind kostenpflichtig.

Zahlungen werden über Telegram und/oder dessen Zahlungsanbieter abgewickelt.

Wir speichern keine vollständigen Kreditkartendaten.

Credits:

- sind nicht übertragbar
- haben keinen Geldwert
- Rückerstattungen können im Falle technischer Fehlfunktionen nach unserem Ermessen gewährt werden

Im Falle technischer Fehler können Credits wiederhergestellt werden.

---

## 7. Geistiges Eigentum

Sie behalten die Rechte an den von Ihnen hochgeladenen Inhalten.

Vorbehaltlich der Einhaltung dieser Bedingungen dürfen Sie generierte Inhalte für rechtmäßige persönliche und
kommerzielle Zwecke nutzen.

Wir garantieren nicht, dass generierte Inhalte keine Rechte Dritter verletzen.

Der Nutzer ist für die rechtmäßige Nutzung verantwortlich.

---

## 8. Haftungsbeschränkung

Soweit gesetzlich zulässig:

Wir haften nicht für:

- indirekte oder Folgeschäden
- entgangenen Gewinn
- Geschäftsunterbrechungen
- Reputationsschäden
- Entscheidungen, die auf generierten Inhalten basieren

Der Service wird „wie besehen“ ohne Garantien bereitgestellt.

Unsere Gesamthaftung ist auf den Betrag begrenzt, den Sie in den letzten drei Monaten vor dem Anspruch an den Service
gezahlt haben.

---

## 9. Beendigung

Wir können den Zugang sperren oder beenden, wenn:

- diese Bedingungen verletzt werden
- Missbrauch festgestellt wird
- gesetzliche Anforderungen dies verlangen

Sie können die Nutzung jederzeit einstellen.

---

## 10. Höhere Gewalt (Force Majeure)

Wir haften nicht für Verzögerungen oder Ausfälle aufgrund von Umständen außerhalb unserer angemessenen Kontrolle,
einschließlich Internetausfällen, Plattformstörungen, behördlichen Maßnahmen oder Ausfällen von Drittanbietern.

---

## 11. Anwendbares Recht

Diese Bedingungen unterliegen dem Recht der Ukraine, sofern nicht zwingende internationale Vorschriften etwas anderes
vorschreiben.

---

## 12. Änderungen

Wir können diese Bedingungen jederzeit aktualisieren.

Die fortgesetzte Nutzung gilt als Zustimmung zu den aktualisierten Bedingungen.

---

## 13. Kontakt

Rechtliche Anfragen:

vatterb0768@gmail.com

---

## 14. Rechtlicher Betreiber

Der Service wird betrieben von:

FOP Glavna Tetiana  
Registriert in der Ukraine  
Kontakt: vatterb0768@gmail.com  
Zahlungen werden über Telegram und dessen Zahlungssysteme abgewickelt.

---

## 15. Keine Partnerschaft

Nichts in diesen Bedingungen begründet eine Partnerschaft, ein Joint Venture oder ein Agenturverhältnis zwischen Nutzern
und dem Service.